...
namespace memory {
    ...
    template<typename T>
    BOOL write_to_process(HANDLE process, LPVOID address, T* buffer) {
        BOOL result = FALSE;
        ...
        result = WriteProcessMemory(process, address, buffer, sizeof(T), &bytes_written);
        if(bytes_written == 0)
    ...
    template<typename T>
    BOOL read_from_process(HANDLE process, LPVOID address, T* out_buffer) {
        BOOL result = FALSE;
        SIZE_T bytes_read = 0;
        ...
        result = ReadProcessMemory(process, address, out_buffer, sizeof(T), &bytes_read);
        if(bytes_read == 0)
